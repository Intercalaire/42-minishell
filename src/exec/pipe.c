...
void handle_heredocs(t_data *data, int i)
{
    ...
    while (data->output->h_doc[i][j] != NULL)
    {
        ...
        // Utiliser un flag pour déterminer si on est dans un terminal interactif
        int is_interactive = isatty(STDIN_FILENO);
        
        while ((line = readline(is_interactive ? "> " : NULL)) != NULL) // Ne pas afficher le prompt si non interactif
        {
            if (g_sig) {
                // Gestion du signal
                free(line);
...
void execute_heredoc(t_data *data, int i)
{
    ...
    int status;
        ...
        printf("Début de execute_heredoc\n"); // Débogage
        pid = fork();
        if (pid == -1)
        {
            perror("fork");
